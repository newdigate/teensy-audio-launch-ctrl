...
namespace newdigate
{
    ...
    class MyLoopSampler : public basesampler<AudioPlaySdResmp, sdsampleplayernote<AudioPlaySdResmp>> {
    public:
        using __base = basesampler<AudioPlaySdResmp, sdsampleplayernote<AudioPlaySdResmp>>;

        MyLoopSampler(SamplerModel &model) : 
            __base(), 
            _model(model),
            _currentId(0),
        ...
        {
            __base::_polysampler.setNoteEventCallback( 
                [&] (uint8_t voice, uint8_t noteNumber, uint8_t noteChannel, uint8_t velocity, bool isNoteOn, bool retrigger) {
